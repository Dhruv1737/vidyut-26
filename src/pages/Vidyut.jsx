...
function PhaseCard({ num, tag, tagGold, title, body, details, highlight }) {
  return (
    <div className={`bg-white p-10 relative overflow-hidden hover:bg-[#fafcf8] transition-colors ${highlight ? 'border-l-[3px] border-gold' : ''}`}>
      <span className="absolute right-6 top-3 font-display text-[5rem] leading-none text-sage2 pointer-events-none select-none">
        {num}
      </span>
      ...
      <h3 className="font-display text-[2rem] tracking-[.02em] text-forest mb-4">{title}</h3>
      <p className="text-[15px] font-light text-muted leading-[1.8] mb-5">{body}</p>
      <div className="flex flex-col gap-[10px]">
        {details.map((d, i) => (
          <div key={i} className="flex items-center gap-[10px] text-[14px] text-muted">
...
function PrizeCard({ rank, medal, amount, label, highlight }) {
  return (
    <div className={`bg-forest text-center p-10 border border-white/[.06] hover:bg-[#1e3824] transition-colors relative ${highlight ? 'border-t-[3px] border-t-gold' : ''}`}>
      <p className="font-condensed font-bold text-[12px] tracking-[.18em] uppercase text-white/30 mb-4">{rank}</p>
      <span className="text-[2.5rem] block mb-3">{medal}</span>
      <p className="font-display text-[3.2rem] text-gold tracking-[.02em] leading-none">{amount}</p>
      <p className="text-[13px] text-white/30 font-light mt-2">{label}</p>
    </div>
  )
}
...
function TimelineItem({ date, name, body, tag, tagGold, done, active }) {
  return (
    <div className="relative pl-8 border-l-2 border-sage2 lg:border-none lg:pl-0">
      {/* dot */}
      <div className={`absolute -left-[9px] top-0 w-[18px] h-[18px] rounded-full border-2 lg:static lg:mb-6 ${
        active ? 'bg-gold border-gold' : done ? 'bg-pine border-pine' : 'bg-white border-sage2'
      }`} />
      <div className="pb-10 lg:pb-0 mt-[-18px] lg:mt-0">
        <p className="font-condensed font-bold text-[11px] tracking-[.18em] uppercase text-muted mb-1 pt-1 lg:pt-0">{date}</p>
        <h3 className="font-display text-[1.9rem] tracking-[.02em] text-forest mb-2">{name}</h3>
        <p className="text-[14px] font-light text-muted leading-[1.75] mb-3">{body}</p>
        <span className={`inline-block font-condensed font-bold text-[11px] tracking-[.1em] uppercase px-[10px] py-[3px] ${
  ...
  return (
    <>
      ...
      <section className="bg-forest px-8 lg:px-14 py-24 relative overflow-hidden">
        ...
        <div className="absolute -left-20 -bottom-20 w-80 h-80 rounded-full bg-pine opacity-[.08] pointer-events-none" />
        ...
        <div className="relative z-10 max-w-[900px]">
          ...
          <div className="flex flex-wrap gap-[10px] animate-fade-up-3">
            {['Sept 9, 2025', 'MANIT Bhopal', 'Tech Track 2.0', '₹18K Prize Pool', 'Open to All Engineers'].map((p, i) => (
              <span key={i} className={`font-condensed font-semibold text-[12px] tracking-[.1em] uppercase px-3 py-[5px] border ${
      ...
      <section className="bg-white px-8 lg:px-14 py-20">
        <div className="max-w-[1100px] mx-auto">
          ...
          <p className="text-[16px] font-light text-muted leading-[1.85] max-w-[620px] mb-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-px bg-sage2 border border-sage2">
            <PhaseCard
      ...
      <section className="bg-forest px-8 lg:px-14 py-20">
        <div className="max-w-[1100px] mx-auto">
          ...
          <p className="text-white/45 font-light text-[16px] leading-[1.85] max-w-[580px] mb-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/[.07]">
          ...
          {/* total bar */}
          <div className="mt-px bg-white/[.04] border border-white/10 p-8 flex flex-wrap items-center justify-between gap-6">
            <div>
              <p className="font-condensed font-semibold text-[13px] tracking-[.15em] uppercase text-white/40 mb-1">Combined Prize Pool</p>
              <p className="font-display text-[3rem] text-white tracking-[.02em] leading-none">₹18,000</p>
            </div>
            <p className="text-[13px] text-white/30 font-light max-w-xs leading-[1.7]">
      ...
      {/* ════════ CAREER ROI ════════ */}
      <section className="bg-pine px-8 lg:px-14 py-20 relative overflow-hidden">
        <div className="absolute -right-16 -bottom-16 w-96 h-96 rounded-full bg-forest opacity-25 pointer-events-none" />
        <div className="max-w-[1100px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
          ...
          <div>
            ...
            <h2 className="font-display text-white leading-[.95] tracking-[.02em] mb-5"
            ...
            <div className="flex flex-wrap gap-2">
              {['Paid Role', 'EV Engineering', 'Real Projects', 'Industry Exposure'].map((t) => (
                <span key={t} className="border border-white/20 font-condensed font-semibold text-[12px] tracking-[.1em] uppercase px-3 py-[5px] text-white/60">
                  {t}
                </span>
              ))}
            </div>
          ...
          {/* right */}
          <div className="bg-black/20 border border-white/10 p-10">
            <p className="font-condensed font-bold text-[11px] tracking-[.2em] uppercase text-white/30 mb-4">Partner Company</p>
            <h3 className="font-display text-[2.2rem] text-white tracking-[.02em] mb-3">Vaayu Robotics</h3>
            <p className="text-[14px] text-white/45 leading-[1.75] mb-7">
            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <div key={text} className="flex items-center gap-3 text-[14px] text-white/55">
                  <span className="w-7 h-7 bg-white/[.08] border border-white/10 flex items-center justify-center flex-shrink-0 text-[14px]">
                    {icon}
                  </span>
                  {text}
                </div>
      ...
      <section className="bg-sage px-8 lg:px-14 py-20">
        <div className="max-w-[1100px] mx-auto">
          ...
          <p className="text-[16px] font-light text-muted leading-[1.85] max-w-[480px] mb-12">
            Three dates. One window to make your mark.
          </p>

          {/* desktop: horizontal line */}
          <div className="hidden lg:grid grid-cols-3 gap-0 relative">
            <div className="absolute top-[9px] left-[calc(16.666%+1rem)] right-[calc(16.666%+1rem)] h-px bg-sage2" />
            <TimelineItem
          ...
          {/* mobile: vertical */}
          <div className="lg:hidden flex flex-col ml-3">
