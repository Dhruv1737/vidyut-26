...
function HeroStat({ num, sup, label }) {
  return (
    <div>
      <p className="font-display text-[1.8rem] sm:text-[2.2rem] text-white leading-none">
        {num}<sup className="text-gold text-[0.9rem]">{sup}</sup>
      </p>
      <p className="font-condensed font-semibold text-[10px] sm:text-[11px] tracking-[.14em] uppercase text-white/35 mt-1">
        {label}
      </p>
    </div>
  )
...
function PartnerCard({ name, role, tag }) {
  return (
    <div className="bg-pine hover:bg-sage2 p-5 sm:p-7 flex flex-col gap-3 transition-colors cursor-default border border-white/[.06]">
      <div>
        <p className="font-condensed font-bold text-[17px] sm:text-[19px] tracking-[.03em] text-white">{name}</p>
        <p className="text-[12px] sm:text-[13px] text-white/45 mt-1">{role}</p>
      </div>
...
export default function Home() {
  return (
    <div className="w-full overflow-x-hidden">
      ...
      {/* ════════ HERO ════════ */}
      <section className="bg-forest min-h-[calc(100vh-80px)] grid grid-cols-1 lg:grid-cols-[1fr_1.05fr] overflow-hidden">

        {/* Left — text */}
        <div className="flex flex-col justify-center px-4 sm:px-8 lg:px-14 py-12 sm:py-16 z-10">
          ...
          <p className="text-white/60 font-light text-[15px] sm:text-[16px] leading-[1.75] max-w-[380px] mb-9 animate-fade-up-2">
          ...
          <div className="flex gap-3 flex-wrap mb-10 sm:mb-12 animate-fade-up-3">
            <BtnGold href="#">Register Now →</BtnGold>
            <BtnGhost href="/event">View Event</BtnGhost>
          </div>

          <div className="grid grid-cols-4 gap-2 border-t border-white/10 pt-6 sm:pt-7 animate-fade-up-4">
      ...
      {/* ════════ PARTNERS ════════ */}
      <section className="bg-sage py-12 sm:py-16 px-4 sm:px-8 lg:px-14 border-b border-sage2">
        <div className="max-w-[1100px] mx-auto">
          <SectionEyebrow light>Official Partners &amp; Sponsors</SectionEyebrow>
          <SectionTitle light className="mb-6 sm:mb-8">Our Ecosystem</SectionTitle>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2">
        <div className="bg-pine px-4 sm:px-8 lg:px-14 py-12 sm:py-16 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 mt-8 sm:mt-10 inline-flex items-center gap-2 border border-white/15 px-[14px] py-[10px] w-fit">
            <span className="w-[7px] h-[7px] rounded-full bg-gold flex-shrink-0" />
            <span className="font-condensed font-semibold text-[11px] sm:text-[11.5px] tracking-[.12em] uppercase text-white/50">
              Official · Govt. of Madhya Pradesh
            </span>
          </div>
        </div>

        <div className="bg-sage px-4 sm:px-8 lg:px-14 py-12 sm:py-16 flex flex-col justify-center">
          <p className="font-condensed font-bold text-[11px] tracking-[.22em] uppercase text-gold mb-5">
            Why It Matters
          </p>
          <p className="text-[15px] sm:text-[16px] font-light text-white/70 leading-[1.85] mb-5">
          ...
          <p className="text-[15px] sm:text-[16px] font-light text-white/70 leading-[1.85] mb-6">
