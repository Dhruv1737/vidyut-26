...
function PillarCard({ icon, title, desc }) {
  return (
    <div className="bg-sage2 p-7 border border-white/[.06] hover:border-gold/30 transition-colors">
      <span className="text-[2rem] block mb-4">{icon}</span>
      <h3 className="font-condensed font-bold text-[18px] tracking-[.03em] text-white mb-2">{title}</h3>
      <p className="text-[14px] font-light text-white/45 leading-[1.75]">{desc}</p>
    </div>
  )
}
...
function MediaCard({ name }) {
  return (
    <div className="bg-sage2 border border-white/[.06] p-6 flex flex-col items-center text-center gap-3 hover:border-gold/30 transition-colors">
      <span className="text-[2rem]">📰</span>
      <p className="font-condensed font-bold text-[17px] tracking-[.03em] text-white">{name}</p>
      <span className="font-condensed font-semibold text-[11px] tracking-[.1em] uppercase text-gold bg-forest px-3 py-[3px]">
        Media Partner
      </span>
    </div>
  )
